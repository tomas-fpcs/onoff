...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handle(MethodArgumentTypeMismatchException ex) {
        final String type = ex.getRequiredType() == null ? null : ex.getRequiredType().getSimpleName();
        var value = ex.getValue();
        String message = String.format("Parameter '%s' should be of type '%s' but the value '%s' is not", ex.getName(), type, value);
        Map<String, String> error = new HashMap<>();
